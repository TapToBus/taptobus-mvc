...
class M_owner_buses{
    ...
    public function add_bus($data){
        ...
        $status = "pending";
        // prepare query
        $this->db->query('INSERT INTO bus (bus_no,root_no,owner_name,capacity,bus_image,permit_image,wifi,usb,tv,status,nic) VALUES (:bus_no, :root_no, :owner_name,:capacity, :bus_image,:permit_image,:wifi,:usb,:tv,:status,:nic)');
    ...
    public function change_conductor($con,$bus_no){
        // prepare query
        $this->db->query('UPDATE bus SET con_id=:con  WHERE bus_no= :bus_no');
    ...
    public function change_driver($dri,$bus_no){
        // prepare query
        $this->db->query('UPDATE bus SET dri_id=:dri  WHERE bus_no= :bus_no');
